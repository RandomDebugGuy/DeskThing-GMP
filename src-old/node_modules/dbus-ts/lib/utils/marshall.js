...
exports.MakeSimpleMarshaller = exports.marshall = void 0;
const put_1 = __importDefault(require("@homebridge/put"));
const long_1 = __importDefault(require("@homebridge/long"));
const signature_1 = require("./signature");
function marshall(signature, data, offset) {
    if (typeof offset === 'undefined')
        offset = 0;
    let tree = (0, signature_1.parseSignature)(signature);
...
function write(ps, ele, data) {
    switch (ele.type) {
        ...
        case 'v':
            ...
            let tree = (0, signature_1.parseSignature)(data[0]);
            if (tree.length !== 1)
...
function MakeSimpleMarshaller(signature) {
    ...
    function checkValidSignature(data) {
        ...
        (0, signature_1.parseSignature)(data);
    }
    switch (signature) {
        case 'o':
        // object path
...
let makeLong = function (val, signed) {
    if (val instanceof long_1.default)
        return val;
    ...
    if (typeof val === 'number') {
        ...
        try {
            return long_1.default.fromNumber(val, !signed);
        }
        catch (e) {
    ...
    if (typeof val === 'string' || val instanceof String) {
        ...
        let data;
        try {
            data = long_1.default.fromString(val, !signed, radix);
        }
        catch (e) {
        ...
        if (data.toString(radix).toUpperCase() !== val)
            throw new Error(`Data: '${val}' did not convert correctly to ${signed ? 'signed' : 'unsigned'} 64 bit`);
        return data;
    }
    // Throws for non-objects, converts non-instanceof Long:
    try {
        return long_1.default.fromBits(val.low, val.high, val.unsigned);
    }
    catch (e) {
...
let checkLong = function (data, signed) {
    ...
    if (signed) {
        if (data.unsigned)
            throw new Error('Longjs object is unsigned, but marshalling into signed 64 bit field');
        if (data.gt(long_1.default.MAX_VALUE) || data.lt(long_1.default.MIN_VALUE)) {
        ...
        if (data.gt(long_1.default.MAX_UNSIGNED_VALUE) || data.lt(0)) {
