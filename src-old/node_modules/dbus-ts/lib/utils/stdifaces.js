...
let stdIfaces;
function stdDbusIfaces(msg, bus) {
...
exports.stdDbusIfaces = stdDbusIfaces;
...
function interfaceToXML(iface) {
    ...
    let dumpArgs = function (argsSignature, argsNames, direction) {
        if (!argsSignature)
            return;
        let args = (0, signature_1.parseSignature)(argsSignature);
        args.forEach(function (arg, num) {
            ...
            result.push(`      <arg type="${dumpSignature([arg])}" name="${argName}${dirStr}" />`);
        });
    };
    ...
    if (iface.properties) {
        for (const propertyName in iface.properties) {
            ...
            result.push(`    <property name="${propertyName}" type="${iface.properties[propertyName]}" access="readwrite"/>`);
        }
    }
...
`;
//# sourceMappingURL=stdifaces.js.map